...
FUNA_PRODUCAO_Quantil_WIDE(Substancia.AMB = Substancia.AMB)[,] %>% FUNA_BARPLOT()
...
#producaoBENEFICIADA[producaoBENEFICIADA$CPF.CNPJ.Titular=='48.277.495/0001-27' & producaoBENEFICIADA$Ano.Base.Ral == 2016 & producaoBENEFICIADA$Substancia.AMB == 'bauxita metalurgica',] <- rep("NULL",21)
 ...
 Substancia.AMB <- 'Niobio \\(Columbita-Tantalita\\)-Secundaria'
 probs <- 0.8
 
 FUNA_PRODUCAO_Quantil_WIDE(Substancia.AMB = Substancia.AMB, 
                         probs = probs, 
                         producao_Bruta = T) |> 
...
# Razão Produção-Venda


df <-    
summarise(
  group_by(producaoBRUTA, CPF.CNPJ.Titular, Substância.RAL, Ano.Base.Ral), 
  "Preço" = round(Valor.Venda.com.Ajuste.por.Substância/
                    Quantidade.Producão.Com.Ajuste, 2),
  "Produção/Venda" = Quantidade.Producão.Com.Ajuste/
                     Quantidade.Venda.com.Ajuste)
  


sort(df[df$`Produção/Venda` > 0,]$`Produção/Venda`) |> plot()
summary(
  df[df$Preço > 0 & 
    is.infinite(df$`Produção/Venda`) == F,]$`Produção/Venda`)

outliers <- 
  boxplot(
 df[df$Preço > 0 & 
      is.infinite(df$`Produção/Venda`) == F,]$`Produção/Venda`)$out |> sort()
...
reserva_groupBY_PROCESSO(Substancia.AMB = Substancia.AMB)
