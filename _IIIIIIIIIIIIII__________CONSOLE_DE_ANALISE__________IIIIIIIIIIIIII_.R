...
ano_inicial <- 2015
ano_final <- 2019
reserva_AMB <- filter(reserva_AMB, Ano.Base.Ral >= ano_inicial, Ano.Base.Ral <= ano_final)
producaoBRUTA <- filter(producaoBRUTA, Ano.Base.Ral >= ano_inicial, Ano.Base.Ral <= ano_final)
producaoBENEFICIADA <- filter(producaoBENEFICIADA, Ano.Base.Ral >= ano_inicial, Ano.Base.Ral <= ano_final)
...
# RESERVA ----
 ...
 #_____Reserva visão ---- 
   Nome.Mina <- '.'
   CPF.CNPJ.Titular <- "."  #'34.456.038/0001-95'
   Substancia.AMB <-  '.'
   Substancia.Agrupadora <- 'Niobio'
   Processo <- '.'

   FUNA_visao_RESERVA(Substancia.AMB = Substancia.AMB, Substancia.Agrupadora, Processo = Processo, CPF.CNPJ.Titular = CPF.CNPJ.Titular, Nome.Mina = Nome.Mina)

   FUNA_RESERVA_Quantil_WIDE(Substancia.Agrupadora = Substancia.Agrupadora)[,] %>% FUNA_BARPLOT()
      ...
      lista[i - 2013]  <- list(
         left_join(
            casos_especiais_MIGRACAO[casos_especiais_MIGRACAO$Ano.Base.Ral == i,
                                     c("Ano.Base.Ral", "Processo", "CPF.CNPJ.Titular", "evento", "Substâncias")],
...
#producaoBENEFICIADA[producaoBENEFICIADA$CPF.CNPJ.Titular=='48.277.495/0001-27' & producaoBENEFICIADA$Ano.Base.Ral == 2016 & producaoBENEFICIADA$Substancia.AMB == 'bauxita metalurgica',] <- rep("NULL",21)
 ...
 FUNA_PRODUCAO_Quantil_WIDE_Agrupadora(Substancia.Agrupadora = Substancia.Agrupadora, 
                                       probs = 0.8, producao_Bruta = F) |> 
   arrange(desc(`2018`)) |> FUNA_BARPLOT()
...
# Substâncias Lista Pareto (Q de Processos)
#   cobre			0	# 1
#   diatomita			0	# 1
#   fluorita			0	# 1
#   ilmenita			0	# 1
